...
- **Paginación: una sola constante, `InventarioApiSidocsa::TAMANO_PAGINA` (1000).** Gobierna dos cosas a la vez —el valor que viaja en `cant` y el corte de la última página en `LectorInventarioErp` (una página con menos registros que ese número es la última)— y por eso no se pueden separar: si el corte viviera en otra constante, subir una sin la otra cortaría el recorrido en la primera página y la sincronización traería solo una parte del inventario sin avisar. `MAX_PAGINAS = 50` es el tope duro contra un endpoint que responda lleno siempre: 50 × 1.000 son los mismos 50.000 ítems que antes daban 500 × 100, holgado contra los ~29.000 del ERP.
